...
#[derive(Component)]
pub struct BlockheightText(u32);

#[derive(Component)]
pub struct BalanceText(u32);
...
pub fn setup(
    ...
    commands.spawn((
        Text2dBundle {
            ...
            transform: Transform::from_xyz(250.0, 310.0, 2.0),
            ..default()
        },
        BalanceText(0),
    ));
...
pub fn pay_button_system(
    ...
    mut interaction_query: Query<
        (&Interaction, &mut BackgroundColor, &mut PayButton),
    ...
    for (interaction, mut color, mut button) in &mut interaction_query {
        match *interaction {
            Interaction::Clicked => {
                // if qr code isn't in off state it means something is going on with it
                        ...
                        *color = PRESSED_BUTTON.into();
                        ...
                        let _task = pool.spawn(async move {
                            let api_response_text =
                                reqwest::get(format!("{}/invoice/50000", server))
                                    .await
                                    .unwrap()
                                    .text()
                                    .await
                        ...
                        info!("be patient! qr should be loading soon")
                    }
                }
            }
            Interaction::Hovered => {
